...
Shader "Crest/Underwater Curtain Slice"
{
	Properties
	{
	...
	SubShader
	{
		...
		Tags{ "LightMode" = "ForwardBase" "Queue" = "Geometry+610" "RenderType" = "Opaque" }
		...
		Pass
		{
			...
			Varyings Vert(Attributes input)
			{
				...
				//const float3 right   = float3(1, 0, 0);// unity_CameraToWorld._11_21_31;
				const float3 up      = float3(0, 1, 0);//unity_CameraToWorld._12_22_32;
				//const float3 forward = float3(0, 0, 1);//unity_CameraToWorld._13_23_33;

				o.positionWS = mul(unity_ObjectToWorld, float4(input.positionOS, 1)).xyz;
